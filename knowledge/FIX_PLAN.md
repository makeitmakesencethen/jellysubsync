...
## Tier 3b — 2.0.24 read-policy follow-ups (3 items)

`R…` items come from the verification work of 2.0.24 (`ReadPolicy`), not from the 2026-09-11 audit.

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| done | **R1** | medium | the cue-indexed route planned one read per located block plus the cluster head and its `MergeGapBytes` came from multiplying the same small-read samples twice, so the gap was ~1,2 KB on every storage and a settled merge decision could not follow the storage at all: the plan was 1600 calls for 800 cues at every profile. Fixed by fitting the two halves of the profile from the two read sizes the pass has measured (`time = latency + bytes / throughput`), so the merge gap is genuinely one round trip's worth of bytes. | `ProfileIsFitted`/`LatencyMs`/`ThroughputBytesPerMs`; unit checks recover 50,00 ms/read and 11,0 MB/s from 2 KB and 40 KB reads where one read size reports 0,04 MB/s. Matrix cell `blocks early` / 1 track: 1600 -> 800 calls at 50 ms/read (a round trip's worth merges the head into its block) and 1600 -> 749 at `blocks late`; unchanged at 1600 where bytes are what cost (`11 MB/s`), which is the correct answer there. Rig at 50 ms/read + 11 MB/s, before (147a89f) -> after: **no change on the rig fixtures** - `arcane` 527 reads/5,53 s, `early` 267 reads/3,07 s, `arcane-4t` 527 reads/5,60 s - because their cue blocks sit ~1 MB apart, above the merge gap, and their location reads are too close in size to separate the two axes (the log now says `one read size only, so throughput is a lower bound` where it used to state 0,1 MB/s as if measured). So: the estimator is correct and self-describing, and the merge decision follows the storage where the pass has the data; on the user's own episode shape it currently has no second read size to fit, which is the open question for the next release. **One behaviour change to weigh on the shared route**: at the synthetic `11 MB/s` profile (fast reads, slow bytes) the fit reports ~0 ms per round trip, so nothing merges and the 32-track plan goes 451 -> 26 400 calls for 67,1 -> 54,3 MB (`blocks late`: 671 -> 20 000). The model's own verdict there is a wash (6,26 s against 6,12 s), and it is the correct answer *given* a 0,05 ms round trip - but it makes that route depend on the measured latency being right: the same 26 400 reads on a share with 1 ms per read would be 26 s against 6,6 s. Cells where the round trip is real (`10 ms/read`, `50 ms/read`, `slow at both`) are unchanged, and fabji's share measures ~10 ms, so it stays merged there. |
| done | **R2** | medium | a plan that is an upper bound rather than an estimate (`PlanWalk`, and a cue plan with clusters to walk) is marked `coarse: true`, and `Compare` then suppresses the miss flag entirely: the expected-vs-actual line for that route can never fail. Measured on the `walk.mkv`/`nocues.mkv` fixtures: plan 62,92 MB / 4 read(s) against actual 0,24 MB / 60 read(s) (bytes 0,00x, reads 15,00x), and on the rig `remux-walk` 545,26 MB / 130 against 1,08 MB / 267. An invisible verification gap. | fixed 2026-09-13 by (b), not (a): the walk stops once it has found the blocks it came for, so its cost is data-dependent and no tight prediction can be derived from the index - a bound tight enough to warn would warn wrongly. `ReadPolicy.BoundNote` (" [bound, not verified]") is appended to every bounded plan's log line and `MkvExtractionStats.PlanBound` counts them, so the gap is visible in the user's log rather than reading as verification. `7409e02` - four new checks per fixture: the marker is on exactly the bounded plans, a fully located plan never carries it, and the walk's cost is held to something that *can* fail (one window per cluster visited: 123 reads for 60 clusters on `nocues.mkv`, where a per-block-header walk would need thousands). 482 checks green. |
| done | **R3** | low | `ReadPolicy.Measured` and `Samples` read the pending sample window, not the pass's measurement state (`Observe` resets `_sampleCount` every 8 reads), so `Measured` was true only in the instant the 8th sample of a round arrived, and nothing called either. | Fixed by renaming and wiring: `PendingSamples` (the window that has not been published yet), `SampleCount` (every read the pass has observed), `MeasuredOnce` and `ProfileUpdates`, and `DescribeProfile` now distinguishes the class defaults from a measurement - `storage not measured yet - deciding from the defaults (0,05 ms per read, 1,5 MB/s); merge gap 0 KB, no read observed` against `storage 50,00 ms per read and 11,0 MB/s (fitted to 2048 B and 40960 B reads, 8 read(s) over 1 update(s)); merge gap 256 KB`. Four checks pin the semantics (nothing read yet, seven reads pending, the eighth publishing, and which estimator is in force). 492 checks green. |

| done | **R4** | medium | the shared multi-track phase was compared against its own raw plan while being served out of the primary track's fetch, so every file with two or more subtitles logged `extract plan: <file> shared-pass expected 0,62 MB/520 read(s), actual 0,00 MB/0 read(s) - this pass missed its own prediction` as a WARN. Netting the plan against what is already in hand under-predicts instead (1 read promised where the phase makes nine window-sized ones), so the line is logged with both numbers plus how much the phase read of its own and the miss flag is not armed: the fetches it lives on were compared by the phase that made them. | Seen in the rig on `arcane-4t` (`missed=1` on every run). Check added: the multi-track fixture must not report a miss. 492 checks green. |
