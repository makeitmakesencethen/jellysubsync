...
| open | **P5-5** | high | **A job whose alignment failed still wrote a synced subtitle into the library** (N1 of the log read-out, verified independently against `subsync.log.1`). 16 jobs' last `ffsubsync alignment:` line carries a **negative score** — the plugin's own text for it is "the engine itself calls a negative score an unsuccessful sync" — and **14 of them completed with an output path**. Three examples, verbatim: job `3a8fbe3d…` (Solsidan S09E01, `12:20:03.959Z`) `score=−41642 offset=−149,990 s against reference subtitle s:0 …` then `completed: … output=…/Solsidan.S09E01.SYNCED.srt bytes=98 change=only 1 cue in a 22-minute file — looks like a forced/signs track, not the full subtitle`; job `155dd60e…` (`−143,800 s`, `bytes=124`, "only 2 cues"); job `14c87ee5…` (`−123,850 s`, `bytes=39853`). Every offset sits at or near the ±150 s window bound, the ruler is a **sibling subtitle** (s:0/s:1) — past `MaxSubtitleReferenceOffsetSeconds` (30 s) that track is documented as "not the same cut" and is to be discarded for the audio — and the run box's `change=` for them says nothing about an offset. The guards that should have caught this (the subtitle-reference ceiling, the window-pin retry, the degenerate-output check) did not fire, and the plugin *knew* the output was degenerate: it wrote the sentence saying so. Not fixed — this pass was read-out only. | `knowledge/LOG_ANALYSIS_2026-09-18.md` §7.7 (all 14, with line numbers) and §7.6; re-verified by `grep -n <job> /subsync-logs/subsync.log.1` for each of the three quoted jobs. |
