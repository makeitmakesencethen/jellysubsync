...
| open | **B18** | low | Four near-identical process runners, two argument styles — and the two styles are not equivalent | **Verified** — the fifth runner, `RunCapturedAsync`, had no call site anywhere in the repo (plugin, `tests/`, the page scripts, `tests/backend/`) and was removed in `4fcd12d` (52 lines), so the row is about **four**: `RunProcessArgumentListAsync` (`SubSyncService.cs:9125`, `ArgumentList`), `RunProcessWithStderrCallbackAsync` (`:9377`, `ArgumentList` plus the stderr callback and the `EngineWatch`/`JobProcessRegistry` bookkeeping), `RunProcessAsync` (`:9181`) and `RunProcessCaptureAsync` (`:9476`), both of the last two building an escaped `Arguments` string. **The styles differ, measured, and that difference is a delta consolidation has to account for**: the same string reaches the child as one argv element through `ArgumentList` and as two through the string runners (`"a b"` → `["a b"]` vs `["a","b"]`; `"\"a b\" c"` → `["a b","c"]`, the string form being parsed by the runtime with no shell involved). Every string-style call site passes a single token today, which is why the difference is invisible in the field, and `EscapeArg` (`:9585`) is what makes that true — it quotes a path with a space so the runtime groups it back into one element. Pinned by three checks in `tests/service_checks.cs` (the ArgumentList pass-through, the string parse, `EscapeArg`'s quoting surviving it) and mutation-verified by `C4-argument-quoting` and `C4-string-quotes` in `tests/backend/mutation_phase0_checks.py`, each caught by the check it targets. Line numbers as of `26c19e8`; the Phase 1 extraction (C4 leaving the class) moves them. |
...
| done | **F19** | medium | `SrtWriter.Render` treats a real 2-second cue as a "guessed" duration | **Fixed `62c3549`.** The guess is a flag now: `SrtWriter.Entry.DurationGuessed` (`Services/SrtWriter.cs:24`), set by the MP4 sample reader when the duration table has no entry for the sample (`Services/Mp4SubtitleExtractor.cs:319`), and `MkvSubtitleExtractor`'s `Cue.DurationGuessed`, set in `AddCue` from BlockDuration (`:1974`) - the Matroska writer has its own copy of the rule and reads the flag too (`:2556`). Checks: 6 in `tests/output_checks.cs` (stated 2 000 ms kept; 1 999/2 001 ms unchanged; a guessed end still clamped; a stated duration that overlaps still clamped; the Matroska copy has both behaviours), and `F19-render` / `F19-mkv-render` in `mutation_phase0_checks.py` are each caught by the check that targets them. Suite 1 020 checks green. **Real, measured 2026-09-18** — see the verdicts section. A real 2 s cue rendered as a guessed duration changes what is written to the user's subtitle. ranked by blast radius; claim not yet read against the code |
...
| closed (refuted by reading, 2026-09-18) | **F20** | medium | `MediaVolume.Of` matches mount points by raw string prefix | Mount matching by raw string prefix can attribute a file to the wrong volume, which mis-prices its extraction and mis-sets its walk ceiling. ranked by blast radius; claim not yet read against the code |
| done | **F21** | medium | The response-body swap in the middleware has no restore on every path | **Fixed `62c3549`.** The capture is inside a `try/finally` that hands `context.Response.Body` back on every exit path (`Api/SubSyncMiddleware.cs:66`, `:103`), and the HTML path writes to the original stream directly. Checks: 6 in `tests/output_checks.cs` - non-HTML, no content type, already-injected, HTML (the script tag is still injected) and a throwing `_next` all restore the body and a later write succeeds; an already-injected page is not injected twice. The `F21-restore` mutation is caught by all six. Suite 1 020 checks green. **Real, measured 2026-09-18** — see the verdicts section. A response-body swap with no restore on every path can serve a corrupted page or script. ranked by blast radius; claim not yet read against the code |
...
| done (not shipped) | **S46** | medium | the wider-window retry was handed an audio reference the plugin had already deleted, so the retry that exists to rescue a window-pinned answer could not start and the job refused. **Found while fixing S22 (2026-09-16) by reading the code the field log named; reproduced deterministically in the harness first, then fixed.** `PrepareAudioReferenceAsync` hands the engine `SpeechCache.CreateReferenceLink(...)` - a symlink `<state>/speech-cache/<key>.mkv` pointing at the media file - and the harvest that follows a run that *analysed the speech itself* dropped that link immediately (`SpeechCache.DropLink` deletes every non-`.npz` file matching the key). The wider-window retry and the verification run of the *same job* are handed the reference captured before that drop (`Jellyfin.Plugin.SubSync/Services/SubSyncService.cs:5863` the first run, `:6229`/`:6273` the retries), so they asked ffsubsync for a file that no longer existed. In the field that was 7 of one run's 8 refusals, every one a job whose answer had reached the search window. **Fix**: the link's lifetime is the job's. The three inline drops are gone and one drop now sits in the job's `finally` (`:6934`), beside the speech gate and the shared extraction directory that are already released there. The harvested `.npz` - the artefact worth keeping - is untouched, and `Prune()` only ever matches `.npz`/`.ref.srt`, so a link that outlives its job is still cleared later. | **Before** (the case with the stand-in engine's reference check turned on - the `check-reference` marker in `tests/fixtures/fake_ffsubsync.sh`, which makes it fail on a reference it cannot open exactly as the real engine does): `Failed`/`Refused`, `runs=2`, nothing written, and the refusal was `refused: the engine could not read the reference it was aligned against (/tmp/speech-cache/23014c11….mkv) · engine said: ffsubsync: unable to read reference …; try ensuring file exists and has correct permissions.` - the field's shape, reproduced. **After**: the same job is `Completed`, `runs=3`, outcome `+3000 ms offset` - the wide retry ran, read the reference, and its answer (not the window-pinned 180 s the first run returned) is what reached the library. Check: `tests/job_checks.cs` case `s46-dropped-reference` (`S46: a job that analysed the audio itself can still use its wider window - the reference outlives the job's retries`); the `S46-inline-drop` mutation in `tests/backend/mutation_job_checks.py` puts the drop back after the first run and is caught by it. Suite 966 checks green. Residual, not fixed: the link now disappears when *its* job ends rather than mid-job, so a second job of the same file overlapping that moment could still have the link deleted under it - narrower than before, and not the field's shape. Removing it entirely would mean not deleting the link at all (one small symlink per analysed file until the cache is cleared) or checking for a running sibling job before the drop. |
...
Folded into P5-10 on 2026-09-19: the walk the engine run attributes is now charged to the read itself
(`Services/SubSyncService.JobPipeline.cs:1341`, `var readTheMedia = !usedSubtitleReference && !usingCachedSpeech;`),
so a run whose speech came from the cache no longer reports a walk of the whole file. **The mirror half is left:**
*VerifyStretchAgainstAudioAsync* reads the film's audio for real (the video is its reference,
`Services/SubSyncService.JobPipeline.cs:2570`) and attributes no walk at all - a media read the volume is never
told about, which is the same accounting gap from the other side. Named here so it is not lost; it wants its own
row if the walk series is to be trusted on a batch with rescales.
...
## New findings this session (2026-09-17 — the mutation driver's own residue)

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| done | **T1** | medium | a mutation driver that is killed mid-run leaves the deliberately broken production source in the tree, so the *next* run reports a false red — and a build or a release made in that state ships the mutation | **Reproduced, and identified by matching failure sets rather than by inference.** Both drivers restore the mutated source only in a `finally` (`tests/backend/mutation_job_checks.py:209-211`, same shape in `mutation_phase0_checks.py`), which a SIGKILL never reaches. Measured tonight: `P8-refusal` resident in `Services/SubSyncService.JobPipeline.cs` makes the job baseline fail exactly one check (`P8: a wrong-cut ruler whose audio retry produces nothing refuses and says nothing was written`); `P10-accepted` resident in `Services/AlignmentMetrics.cs` makes it fail exactly one (`P10: a PAL-like rescale with correction on is written, and the outcome names the factor`) — together the exact two-name red reported twice in this session. For contrast, `P8-stale` and `P8-discard` residue each fail that P8 check **and** `S45: the audio retry writes to its own path…`, a pair never seen. Clean tree (`git status` empty, `1701f6e`): job baseline green 6/6 runs, Phase 0 baseline `39 pass, 0 fail`, `python3 tests/run_checks.py` `1008 pass, 0 fail` (exit 0, run with the harness's own `LD_LIBRARY_PATH` and `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT`). Fix, not written: signal-safe restore (SIGINT/SIGTERM/SIGHUP handlers, or build the mutant from a copy) and refuse to start when `git status --porcelain -- Jellyfin.Plugin.SubSync` is non-empty. |
| open | **T2** | low | `run_checks.py` needs the harness's own environment or it fails in a way that reads like a test failure: with `LD_LIBRARY_PATH` unset the .NET host aborts with `Couldn't find a valid ICU package installed on the system` (exit 1) before any check reports | Seen tonight: a bare `python3 tests/run_checks.py` printed the B8 checks, then `FailFast`/ICU text and exited 1; the same command with `LD_LIBRARY_PATH=/opt/data/local/icu/usr/lib/x86_64-linux-gnu` and `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1` is `1008 pass, 0 fail`. The drivers set both themselves (`mutation_job_checks.py:118-133`); the suite's own README does not say so, so the failure mode is "the suite is red" when it is an environment gap. Fix, not written: have `run_checks.py` set the same two variables for its child processes, or fail with a sentence naming the missing ICU path. |

## New findings this session (2026-09-16 — the Phase 0 characterization pass)

Three new rows from writing the Phase 0 checks (`tests/service_checks.cs`), which covered C4, C8 and C9 before
their extraction; the fourth finding of that pass was B18's own row, updated in place above (count 5 → 4, the
measured argument-style delta). **Line numbers below are as of `26c19e8`**, the revision the checks were written
against — the Phase 1 extraction moves this code out of `SubSyncService.cs`, and whichever commit moves it
re-points these rows.

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| open | **F31** | medium | the install status can report the version of an engine that is not the one that will run | **Verified** — `GetInstallationStatusAsync` fills `IsInstalled` and `ResolvedBinaryPath` from the resolver (`SubSyncService.cs:7481` → `:689`) but fills `FfSubSyncVersion` by probing the *managed* path (`:927`, `RunProcessCaptureAsync(ManagedFfSubSyncPath, "--version", …)`), so the two halves of the same status describe different binaries. Measured in the harness: with a configured engine at another path that answers `ffsubsync 9.9.9` and a managed binary that answers `ffsubsync 0.5.1`, the status reports `ResolvedBinaryPath` = the configured path, `IsInstalled` = true and `FfSubSyncVersion` = **`ffsubsync 0.5.1`** — the managed binary's, i.e. not the engine a job would execute. Nothing syncs with that string, so it is a misleading report rather than a wrong result (the D12 family); severity medium on that basis. Pinned by `tests/service_checks.cs` ("the version field quotes the managed binary, not the engine a job would actually run"); the mutation that probes the resolved engine instead (`C9-version-source` in `tests/backend/mutation_phase0_checks.py`) is caught by it.  **Note (Phase 2, 2026-09-16):** the split moved this code - the status is now `FfSubSyncEngine.GetInstallationStatusAsync` (`Services/FfSubSyncEngine.cs:252`), whose version probe is at `:313` against `ManagedFfSubSyncPath` (`:56`) while `ResolveFfSubSyncPath` (`:75`) feeds `ResolvedBinaryPath` (`:333`); the line numbers quoted above are from the pre-split revision. |
| open | **B32** | low | `RunProcessCaptureAsync` takes no cancellation token, so the install's provisioning steps cannot be cancelled by the caller's token | **Verified** — `SubSyncService.cs:9476` declares `RunProcessCaptureAsync(string executable, string arguments, string? workingDir)` with **no token parameter**, and it is what runs `python3 --version` (`:910`), the managed engine's `--version` (`:927`), `python3 -m venv` (`:988`), `id -u` (`:9533`), `apt-get update` (`:9544`), `apt-get install` (`:9550`) and the ensurepip probe (`:9572`). The controller hands `HttpContext.RequestAborted` to `InstallFfSubSyncAsync` (`Api/SubSyncController.cs:598`); the pip steps honour it (`RunProcessAsync` takes a token) and these do not, so abandoning the install request leaves the venv creation, the apt-get pair and the probes running to completion. Nothing leaks: all five runners register in `_liveProcesses` (`:9547`) and are therefore stoppable by `Kill` and by the teardown (B14) — the gap is that the intended path has no effect. Low: wasted work after an abandoned request, no wrong result. Pinned as a contract by `tests/service_checks.cs` (the capture runner's output and exit code; the missing-binary spawn failure); the `id`-based refusal path is mutation-verified (`C9-non-root`).  **Note (Phase 2, 2026-09-16):** the runner is now `SubSyncProcesses.RunProcessCaptureAsync` (`Services/SubSyncProcesses.cs:347`, no token parameter and no kill registration - the third runner has one at `:291`); its provisioning call sites are in `Services/FfSubSyncEngine.cs` (`:296`, `:313`, and the `python -m venv`/apt-get steps). |
| open | **F32** | low | the sweep's skip counters do not account for the embedded tracks it skips | **Verified** — `SweepLibraryAsync` skips a track that is not external, or whose external path is empty, at `SubSyncService.cs:4428` (`continue;` with **no counter touched**), so `SkippedOther` counts only external files that are missing or have no synced version. Measured in the harness with a file carrying four subtitle tracks (one external with a cached synced output, one external with a fail streak past the limit, one embedded, one external whose file is gone): the sweep reports `ScannedItems=1, SkippedCached=1, SkippedFailed=1, SkippedOther=1, CandidatesEnqueued=0` — three of the four tracks appear in the counters and the embedded one appears nowhere, so "what the sweep did with this file" cannot be answered from the numbers. Low: a reporting gap, no work lost and no wrong decision. Pinned by `tests/service_checks.cs` ("the sweep scans the library and counts what it saw"); mutations `C8-sweep-scanned`, `C8-sweep-cached` and `C8-sweep-failstreak` are each caught by it.  **Note (Phase 1, 2026-09-16):** the sweep is now `SubSyncService.SweepHistory.cs` (`SweepLibraryAsync` at `:518`, the embedded-track skip at `:556`); the line numbers quoted above are from the pre-split revision. |
| open | **F33** | low | a cancellation that lands between `TrackChildProcess` and the token registration leaves the child process running *and* untracked | **Verified by code order and by a check that caught it** — each of the three runners that take a token starts the process, registers it in `_liveProcesses`, and only *then* arms its kill callback: `Services/SubSyncProcesses.cs:160` (`process.Start();`), `:161` (`TrackChildProcess(process);`), `:164` (`cancellationToken.Register(…)`) — the same order again at `:211`/`:212`/`:215` and `:279`/`:280`/`:291`. A cancel that arrives in that window ends `WaitForExitAsync(token)` (so the caller sees a cancellation) but does not run the kill, and the runner's `finally` then removes the pid from the table (`:179`) while the process is still alive — so neither `Kill` nor the teardown (B14) can reach it, and what is left running is a real engine or ffmpeg run rather than a sleep. Observed, not deduced: the C4 cancel check failed with `cancelled=True childPid=… alive=True cmdline='/bin/sleep 30' trackedBefore=[] trackedNow=0` - the process it watched was its own child (not another check's), it reads as `/bin/sleep 30` while still alive, and the tracked table was already empty again by the time the assertion ran. Measured frequency: one full-suite run in six under load, with the cancel issued 250 ms after the pid appeared; the shortfall is visible per run now that the check runs five attempts and reports `killed=n/5` (observed 5/5, 5/5, 4/5, and the miss's own detail ends `cmdline='(gone)'`, i.e. that child exited a beat after the poll gave up). Three candidate mechanisms, and from outside the code none can be ruled out: the callback was not armed yet, the kill threw and `catch { }` swallowed it, or the registered callback was delayed by thread-pool starvation while the suite loads every core (which the `(gone)` detail favours). All three leave the same state - a child running while the table no longer holds it - and all three are fixed by killing in the `finally` when the token is cancelled, independently of the callback. Narrow (the window is a couple of statements wide, and the work it orphans finishes on its own) which is why it is low. **Not pinned by a check today, on purpose:** the check now waits past that window before cancelling, so it asserts the guarantee for a cancel that arrives after arming; a fix (arming the callback *before* `Start()`, or killing in the `finally` when the token is cancelled) would need its own mutation, since the check as written cannot distinguish a fixed window from an avoided one. |

## Verdicts read against the code (2026-09-18 — F19, F20, F21, and the tooling rows T1/T3)

Three open mediums were read against the shipped code and *measured* rather than ranked, because they are the
ones that could write a wrong artifact or break the web client. Plainly: **F19 and F21 are real, F20 is not.**

**F19 — confirmed, fixed in `62c3549`.** `SrtWriter.Render` calls a cue whose duration is exactly 2000 ms "guessed"
(`Services/SrtWriter.cs:35`, `var guessed = entry.EndMs - entry.StartMs == 2000;`) and `guessed` is a **disjunct
of the overlap test** (`:36`), so the clamp is forced for that cue. Measured with the shipped method: entries
(0 ms, 2000 ms) and (5000 ms, 7000 ms) render `00:00:00,000 --> 00:00:04,999` — the real two-second cue stretched
to the next cue minus 1 ms — while the controls 1 999 ms and 2 001 ms render unchanged (`00:00:01,999`,
`00:00:02,001`), which pins the trigger to the exact number. The ambiguity is structural, not a fixture:
`Services/MkvSubtitleExtractor.cs:1974` writes `start + durationTicks` when the block **has** a duration and
`start + 2000` when it does not, so a block whose real duration is 2,000 s arrives indistinguishable from the
fallback — and `:2556` carries the same heuristic inline, so the Matroska path has its own copy and a fix has to
land in both places. Blast radius: the cue lingers up to ~5 s longer than the source says, nothing is lost or
mis-addressed. Fix, not written: carry "no duration was given" as a flag on the entry instead of inferring it.

**F20 — refuted as stated, closed.** The claim was a wrong-volume attribution from raw prefix matching. Read:
`MediaVolume.Mounts()` sorts the mount list **longest mount point first** (`Services/MediaVolume.cs:83`) and `Of`
returns the first match (`:31-37`), so a path under the more specific mount point is attributed to it —
`/mnt/disk10/...` is matched by `/mnt/disk10` before `/mnt/disk1` can see it. Misattribution would need the wrong
mount point to be **both** longer than the right one and a prefix of the path, and a path that begins with a mount
point's own string genuinely lives on that mount. Residual, real but not the claim, and **not given a row**: when
*no* mount matches — a library root that is a symlink, since `Path.GetFullPath` does not resolve symlinks — the
fallback returns `Path.GetPathRoot`, i.e. `/`, so every such file shares one key and the scheduler's spreading
preference stops distinguishing them. Jobs still run; they just no longer spread by volume. Caveat on the
instrument: `MediaVolume` reads `/proc/mounts` directly with no seam, so this verdict is from reading, not a fixture.

**F21 — confirmed by measurement, fixed in `62c3549`.** Read: `InvokeAsync` saves `originalBodyStream` and assigns
`context.Response.Body = responseBody` (`Api/SubSyncMiddleware.cs:60-62`), and only the HTML path restores it
(`:99`). The two early returns — a non-HTML content type (`:70-72`) and an already-injected body (`:82-84`) — copy
to the original stream and leave `Response.Body` on the `MemoryStream`, which the `using` then disposes; there is no
`try/finally` anywhere, so a throwing `_next` leaves the same state. Measured with the shipped class on a
`DefaultHttpContext` for `/web/index.html`: `non-html response`, `no content type at all` and `the next middleware
throws` each report `restored=False body=MemoryStream`, and a later write to `Response.Body` throws
**ObjectDisposedException**; the HTML path reports `restored=True`. Blast radius: any non-HTML answer to `/`,
`/index.html` or `/web/` — an error page, a redirect, an auth challenge — leaves the body pointing at a disposed
stream, so an outer exception- or status-code middleware writing its own body gets an `ObjectDisposedException`
instead of a page. Fix, not written: wrap the capture in `try/finally` and restore the stream on every path.

**T1 — done (`26d666a`).** `tests/backend/mutation_residue_guard.py` (check mode refuses a production tree that
differs from HEAD, naming the file; `--selftest` plants each known residue, proves the baseline shows it and
restores), and both drivers refuse to start on a dirty tree and install SIGINT/SIGTERM/SIGHUP handlers that put the
source back. Measured: the precheck refuses while a driver holds a mutation (exit 2, file named); a SIGTERM sent
mid-sweep prints `signal 15: source put back: True` and leaves `git status` clean; `--selftest` passes for
`P8-refusal` and `P10-accepted` and leaves the tree clean.

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| open | **T3** | medium | the job mutation driver's *sweep* accumulates state, so a check can begin failing for reasons unrelated to the mutation under test, and its failure sets therefore cannot be used to identify a resident mutation | **Measured 2026-09-18, five runs** (`tests/backend/mutation_job_checks.py:209`, the sweep loop; `tests/backend/mutation_residue_guard.py`). **Measured 2026-09-18, five runs.** One mutation per process, clean tree: `P3-cache-hit` fails exactly its own P3 check (`P3: the file's second job is handed the stored analysis…`), while the sweep reported the same mutation failing the `P10` accepted-twin check as well; `P10-accepted` fails the accepted-twin check in a fresh process (three runs) while the sweep reported it against the *refused* twin (`P10: a rescaled result that is not a framerate pair is refused…`); and the sweep shows the accepted-twin check failing for fifteen of the twenty-five mutations, all late in the sequence, where the first five show single failures with no P10 at all. Nothing is wrong about *whether* a mutation is caught — 25/25 and 43/43, both baselines green — but the set of catching checks is not stable between a sweep and a single-mutation run, which is exactly what the residue investigation leaned on before this was measured. `mutation_residue_guard.py`'s `KNOWN_RESIDUES` are therefore measured one-mutation-per-process and its comment says not to take them from the sweep. Fix, not wanted yet: the sweep's value is coverage, not attribution. |

## New findings this session (2026-09-18 — the GUI/UX pass)

The user's design/UX pass. Section A is an investigation before any code is touched: every claim is read
against the code and, where the field can answer it, against the plugin's own log, and each fix is its own
commit carrying before/after evidence.

**Shipped as 2.0.63 (beta)** — the whole pass, B1–E4 plus G1, in one release (`7dfa3eb`, pushed to `beta`;
`tests/backend/verify_release.py 2.0.63.0` passed: catalog reports 2.0.63.0, the 42 519 030-byte zip's MD5 is
`50f90f953a245c60de2a4296517b950d` and matches the catalog, the packaged meta.json and DLL both carry 2.0.63.0,
174 bundled ffsubsync files, 23/23 refactor markers plus this batch's own markers present and both removals
confirmed). The stable/root catalog is a `beta` → `master` merge away and was left for the user.

**After the live build, three follow-ups from the user's own testing (each its own commit, not yet released):**

- `ecd0f31` — the run box's two lines said every number twice (`parallel · 1/64 workers · 1/9 · 1 failed` over
  `8 subtitles left · 1/9 done (11%) · 1 failed · time left: estimating`). The workers line now says what the
  workers are doing and the counts line says how the run is going, "N subtitles left" is gone because the
  fraction states it, and the **time estimate is removed rather than fixed**: on the user's 489-task run it read
  "about 4 min left" at 29% and "about 10 min left" at 46%, because a run mixes 25-minute episodes with
  two-hour films. The worker rows and phase line moved above the counts line. Measured across a whole run
  (`tests/gui/runbox-lines.js`): no sample containing "min left", "time left" or "subtitles left".
- `2d17e75` — the per-file results area (`#ss-log`, the C2 list under the counts line) is removed from the run
  box, along with `logLine`/`logTask`/`logTerminal`. What is left is the live run: workers line, bar, worker
  rows, counts line. A run's record is History, which renders the same rows from the same renderer. History was
  re-checked after the removal: 26 runs, day headings, a detail with 9 rows and the problems-only switch, and
  Copy as text still yields the old log shape.
- `b0d6710` — the run box was a sibling of the three panels, so History, Settings and Sync all showed it; it is
  now the first child of `#panel-sync` and is absent (no space, no text) on the other tabs while a run goes.

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| done | **G1** | medium | A sync started from a **detail page** — the single-item "Sync Subtitles" dialog — never appears in History, while every sync started from the plugin's own page does. Not a second pipeline: both entry points reach the same `EnqueueSync`, the same `_jobs` and the same FIFO pump, so the job runs, reports and writes its sidecar. What differs is the **grouping key**. The detail page's single-item action POSTs `/SubSync/Sync` (`Web/subsync.js:785`) and `StartSync` passes `batchId: null` (`Services/SubSyncService.Queue.cs:39-49`, `:45`); everything else POSTs `/SubSync/Batch` with a label and therefore carries a `BatchId` — the menu page's "Sync selected" for any number of items, one included (`Web/subsyncMain.js:1215-1223`, `postBatch`), and a *series or season* detail page's "Sync all episodes" (`Web/subsync.js:613`). History is built only from jobs that have a batch: `GET /SubSync/Batches` (`Api/SubSyncController.cs:376-403`, `:381`) asks `GetBatchIds()`, which filters `j.BatchId is not null` (`Services/SubSyncService.Queue.cs:590-597`, `:593`), and the file that survives a restart is written from `SnapshotBatchHistory()`, which filters `!string.IsNullOrEmpty(j.BatchId)` (`Services/SubSyncService.SweepHistory.cs:362-365`, `:365`) — so a standalone job is invisible to the tab **and** to `batch-history.json`, and cannot come back after a restart. It *is* visible while it runs (the dialog polls `/SubSync/Jobs/{id}`, `Web/subsync.js:785-793`; the menu page's worker panel reads `/SubSync/Active`, `Api/SubSyncController.cs:745-762`), which is why it reads as "the sync ran and then vanished". **Confirmed in the field three times, all three of them detail-page single syncs, against 23 batched enqueues:** `queued: job=67dd7586… item=f28d7e91… mode=auto batch=(standalone) … video=…/Mr.Robot.S03E04.eps3.3…mkv` (2026-09-16 00:59:29), preceded by `Clara Sola` (2026-09-12 15:43:39) and `Hereditary` (2026-09-14 11:13:15). All three logged `completed` with an `output=…SYNCED.srt`, and the restart 42 minutes after the third reported `batch history: restored 7 batch(es), 284 task(s) from /config/data/data/subsync/state/batch-history.json` — the same 7/284 as every restore since 2026-09-14, i.e. the finished standalone job added nothing to the file. Not S14-class: item, track and job are identical on both surfaces and only the group is missing. **Fix, not written — the direction is the user's call:** (a) server-side, let the read models treat a batchless job as its own one-task entry (label = the item, id = the job id) so History shows every run and nothing else moves; or (b) client-side, have the detail-page dialog send a one-task batch. Either way `BatchHistory.MaxBatches = 20` (`Services/BatchHistory.cs:97`) raises the follow-up — whether a single sync should consume a history slot a 300-task run would otherwise hold — and that is a decision, not a default. **Direction chosen by fabji 2026-09-18: (a) - the read model, with no batch invented anywhere. Fixed in `8d31b17`.** A run of one is addressed by a derived id (`Services/RunId.cs`, `single:<job id>`) instead of by a batch minted at enqueue time: `GetBatchIds()` and `SnapshotBatchHistory()` both enumerate the batchless jobs, `GetBatchJobs()` resolves that id to the one job (`Services/SubSyncService.Queue.cs:579-617`), and the row is titled by the item (`Label = label ?? (string.IsNullOrEmpty(batchId) ? video.Name : null)`, `Services/SubSyncService.Queue.cs:235-238`, with the view falling back to it at `Api/SubSyncController.cs:507-510`). **The rotation question the user asked before this was written is answered: one entry per run would have consumed a slot of the 20 exactly as (b) would, because the bound counts entries and a run of one is an entry - so it is NOT a (b)-only effect.** `BatchHistory` therefore bounds the two kinds separately (`MaxSingleRuns = 20` beside `MaxBatches = 20`, `Services/BatchHistory.cs:96-107`, applied by `Bound()` at `:170-183`). **Measured on the rig, before and after, one probe against the same fixture and the same server** (`tests/backend/g1_history_probe.py`): against the 2.0.59 build the standalone job ran and logged `completed` yet was absent from `GET /SubSync/Batches` (21 runs listed and not that one), `GET /SubSync/Batch/single:<job>` answered **404**, and the history file did not hold it - three failures; against this build every check passes: the row is listed as `{"Id": "single:7c3f…", "Label": "Helikopterrånet S01E01", "Status": "Completed", "Total": 1, "Ok": 1}`, its view answers 200 with one task, the file holds it, and after a restart the list returns 20 batches + 3 single runs with that row intact (its label, counts and view included). The one-task batch control is listed identically in both builds, so the change did not make the list say "everything is a run". Suite: `python3 tests/run_checks.py` exit 0, 1025 PASS, no FAIL - three C# cases (the derived id cannot be a batch id; a run of one survives the round trip; the rotation keeps 20 of each) and one source pin that both read models derive the id. |
| done | **G2** | high | **An image-based subtitle track (Blu-ray PGS, DVD VobSub, DVB, XSUB) could be selected and queued from the library page, and every such click became a *failed task* of the run.** The engine can never align one - `LanguageSupport.IsImageBased` covers `pgs`, `dvd`, `vob`, `xsub`, `dvb`, `hdmv`, and `ImageBasedRefusal` turns that into the sentence the user sees - so `EnqueueSync` throws on one on purpose (`Services/SubSyncService.Queue.cs:151`: a visible failed task, not a silent drop), and `CreateBatch` records the throw as a `SyncJobStatus.Failed` job carrying that sentence, logging only `LogWarning("Batch {BatchId} task {Index} failed validation: {Error}")` (`Services/SubSyncService.Queue.cs:317-372`) - into **Jellyfin's** log, not this plugin's. That is why `/config/data/data/subsync/subsync.log` has no image line at all and the defect is only visible as a number in the run ("120 failed"). **Found in the user's own rig log, five times in one afternoon of testing (2026-09-18 04:38:53, 04:39:56, 04:40:29, 04:41:33, plus my probe at 05:24:37):** `Batch "b258342a…" task 3 failed validation: "This track is DVDSUB, an image subtitle format: the engine aligns text, so it cannot be re-timed here…"` - each from a fresh 1-7-task batch built by the interface, and the batch at 04:41:33 was `7 task(s) across 2 file(s)`. **Root cause: the page never used the flag it was given.** The listing shows a bitmap track with `UnsupportedReason` deliberately (S5 - hiding one made the list disagree with the file) and `Web/subsync.js` (the detail dialog, B1) filters on it, but `Web/subsyncMain.js` mentioned it nowhere: `trackOptionsHtml()` listed the track as an ordinary option, `"All N tracks"` counted it, `bestPerLanguage()` could pick one as its language's only track, `discoverLangOptions()` offered a language carried only by one, `movieRowCount()`/`subtitleButtonLabel()` counted it, and both `buildTasksForItem()` and the scope path (`episodesInScopeChain`) handed it to the queue. The sweep had the same hole for bitmap **sidecars** (`.sup`, DVD `.sub`/`.idx`), and its own comment claimed image tracks "are left out entirely" - which `ListSubtitles` never did (`Services/SubSyncService.SweepHistory.cs:155-160`). **Fixed in `b7f3bcf`:** one predicate (`trackUnsupported`/`syncableTracks`) now guards every path that builds a queue or counts what a button would queue; the picker still lists the track, disabled, with the reason in `title`; the row's Sync button is disabled with that reason when nothing can be queued; "nothing found" and "only image formats" are different messages; and the sweep skips such a track with its own counter (`SkippedUnsupported`, in the result and in the sweep's summary line). **Measured on the rig's own fixture `Bitmap` (exactly one track, index 2, DVDSUB), driven through the page and not the API** (`tests/gui/g2-bitmap-row.js`): before, `tasks the page would post: 1 → [2]` and the batch came back `total=1 ok=0 failed=1` with the refusal as the task's error (`tests/backend/g2_bitmap_probe.py --expect-refusal`, batch `d8f0d107…`); after, the row offers `All 0 tracks`, its DVDSUB option is `disabled` with the full reason, its button reads `Sync subtitles` and is disabled, and pressing it creates **0 batches and sends no request to the server** - while the control file (`Embedded Test`, five text tracks) still offers `All 5 tracks`, all five selectable, `Sync 5 subtitles` enabled, so the filter is not wider than the defect. Runtime cross-check in the rig's Jellyfin log: 12 refusals in total, the last at 05:43:06 from a build predating the fix (the fixed DLL was written at 05:46:01), and **0 after it** across the probe runs that had produced one every time before. Suite: `python3 tests/run_checks.py` exit 0, 1038 PASS, no FAIL - the new C# case (a bitmap sidecar is skipped as an image format instead of queued) plus source pins for the page's predicate, every call site, the sweep's skip and counter, and the corrected comment. |

**Priority 4 (2026-09-18, the user's live testing of 2.0.64) — a task that has not finished was drawn as a FAILURE.**

`buildTaskRow` decided a result row's state word and colour with a three-way ternary:
`status === 'Completed' ? 'Synced' : (status === 'Cancelled' ? 'Skipped' : 'Failed')`, and
`status === 'Completed' ? 'ok' : (status === 'Cancelled' ? 'skip' : 'bad')`. "Everything else" is where
**Queued and Running fall**, so every task that had not started yet was painted **Failed in red**. Opening a
run in History while it was still going — the natural thing to do on a 489-task run — told the user the run
had failed where nothing had gone wrong, which is why this was fixed before the three cosmetic items.

Measured against the shipped code, `buildTaskRow` lifted out of `Web/subsyncMain.js` and run in node:
`Queued -> "Failed" (ss-task ss-task-bad)`, `Running -> "Failed" (ss-task ss-task-bad)` — the two statuses a
pending run is made of. On the rig, a ten-task batch sampled every 250 ms through `GET /SubSync/Batch/{id}`
spent its entire life with 1–10 tasks `Queued`/`Running` (0.0 s `{Running:2, Queued:8}` → 2.8 s
`{Completed:7, Failed:2, Running:1}`), i.e. 1–10 rows reading "Failed" at every moment of the run
(`tests/backend/priority4_queued_failed.py`). The **summary chip was never wrong** — checked and refuted
before writing anything: the page tests `Queued`/`Running` *before* it tests failures, and a batch followed at
200 ms reported `Queued → Running → Partial` with no false failure in any sample. The defect was only ever in
the per-task rows.

Fixed by one table with a row per status the server reports (`Completed`→Synced/ok, `Cancelled`→Skipped/skip,
`Failed`→Failed/bad, `Refused`→Refused/skip, `Running`/`Queued`→pending-neutral) behind `taskState(status)`,
plus a `.ss-task-pending` style in the page's muted colour — and a status the table has never seen is
**neutral, never red**, because an unknown word must not be an accusation. A pending row's note no longer
reports its own status word as if it were an outcome: it shows the phase the server carries ("Queued —
starting as soon as a worker is free"), which is the answer to "why is this waiting".

Verified in a real browser both ways. Before (2.0.64 DLL on the rig): a pending row's state word resolved to
`rgb(255, 107, 107)` — the failure red. After: the same probe on the fixed build opened History mid-run and
reported the rows as `Queued`, `Queued`, `Queued`, `Running`, `Running`, `Queued`, with the pending words
computed as `rgba(255, 255, 255, 0.55)` (muted) and **zero** pending rows red — while the run's four genuine
failures still read `Failed` in red. `tests/gui/priority4-colour.js` reads the *computed* colour, so "not red"
is a measurement of what the browser painted, not a claim about a stylesheet rule.

Suite: `python3 tests/run_checks.py` exit 0, **1040 PASS**, no FAIL — two new checks, and the behavioural one
is **mutation-verified**: putting Queued back on the old word/class makes it fail
(`FAIL a task that has not finished is never drawn as a failure (priority 4)`). It runs the shipped function
by lifting the three pieces out of the page source, because the defect was a *mapping* and a source pin
cannot see a mapping.

## Pass 5 (2026-09-19) — the selection marking, the run-box lines, the refusal sentence, and the day's log

Four asks in one pass: a visual regression in the selection, two run-box lines swapped and enlarged, one confusing
refusal sentence, and two read-outs (the day's log, and a real desync report). Each fix is its own commit and carries
before/after evidence; the read-outs produced two artifacts and no code change:
`knowledge/LOG_ANALYSIS_2026-09-18.md` (the day's log, machine-readable counts in
`tests/backend/log-analysis-2026-09-18.json`) and `knowledge/INVESTIGATION_false_trail_2026-09-18.md`.

| state | id | sev | what | evidence / commit |
|---|---|---|---|---|
| done | **P5-1** | medium | **The reported regression does not reproduce as reported, and the real one is next to it.** The ask was "a single right-click pick no longer gets the visual highlight that selected rows used to show". Measured against the shipped build, a single right-click pick **does** get it: `class="ss-row selected"` → the theme tint, the 3 px primary inset bar and the primary bold name, computed identically at one pick and inside a two-pick selection, driven with Playwright's own mouse (`page.click({button:'right'})`) and re-measured in six states (first row, a row already viewed, a Series row, after a search re-render, after a library change, after Select all). The class was never tied to the click handler: `render()` applies it from `selectedIds`, the same map the right-click writes, and that line is unchanged since `9748913` (2026-09-12) — `git show 8927960:…/subsyncMain.js` carries the identical `(isPicked ? ' selected' : '')`. What the right-click-only fix *did* remove is the marking on the row a user **clicks**: before it, a click also picked, so the clicked row carried the pick class; after it, `isSel` only decides whether the row's options render and the row carried no state at all — and because the list reorders only from the second pick, "one row clicked" was the one case with no marker anywhere. Fixed by giving the viewed row its own marker (`.ss-row.viewed`: a 5,5 % white tint, a 3 px neutral inset bar, a 600-weight name), declared **before** `.ss-row.selected` so a row that is both reads as picked; the pick's own marking is untouched, so a single pick still looks exactly like a multi-pick. **Reversed on 2026-09-19 (P5-11):** the neutral tint renders grey-black on the dark theme, and the user asked for the pick colour - "the single episode marked look is black which is wrong, selected files should always have the same colour blue". The viewed row now wears the theme primary, exactly as the pick does; the pick marking itself is unchanged. | `tests/gui/p5-viewed.js` (real input, before/after), `tests/gui/p5-realclick.js` (real right-click, before), `tests/gui/p5-hunt.js` (six states, before), shots `tests/gui/shots/p5-{before,after}-look-{1-clicked,2-picked}.png` and `p5-before-select-*.png`; new suite pins "the viewed row has its own marker, defined before the pick class". |
| done | **P5-2** | low | The run box's counts line ("24/772 done (3%) · 22 already in sync") was printed **last**, below the worker rows and the phase, in `.82rem`; the workers line ("parallel · 8/8 workers") sat at the top in the page's own size. The counts line is now the box's first line, the workers line follows it, and both are a step larger — measured in the browser: counts 12,2 px → 14,1 px, workers 14,9 px → 15,6 px. Every field is kept: the counts line still names failures and no-ops ("5/8 done (63%) · 1 failed · 2 already in sync" measured mid-run). The bar, the worker rows and the phase line keep their places. | `tests/gui/p5-probe.js` before/after (`p5-{before,after}-probe.json`), shots `tests/gui/shots/p5-{before,after}-runbox.png`; suite pin "the counts line is the run box's first line, above the workers line (priority 5)". |
| done | **P5-3** | medium | The "unverified, nothing written" refusal sentence had three defects the user named: it advised syncing "against a subtitle track if the file has one" in the same breath as saying the file holds no other text track; its final sentence did not parse; and it mixed the three separate facts. Rewritten in `Services/SubSyncService.JobPipeline.cs` as three sentences — what happened (this track is inside the file, the film's audio was its only reference, with the measured detail), why it is not trusted (nothing to cross-check it, a second or more out on a short file), and what to do (run it again from an external `.srt` beside the video, where the audio is the reference the plugin is meant to use and a sidecar is written). The old wording also **claimed more than the code knows**: the ruler is the audio when no sibling track could be *built* (signs track, unreadable text, wrong cut), not only when the file has none, so the new sentence says "there was no subtitle track to check that answer against". | `tests/job_checks.cs` — the P13 terminal-G check now pins the new opening and a second check pins the three-part shape and forbids the old advice ("if the file has …", "Sync it against a subtitle track"); S30's own row (a refusal rendered as a failure) still stands and is unaffected. |
| open | **P5-4** | high | **False Trail (2011) / Jägarna 2 reported still out of sync after a sync that says +50 ms.** The log rules the known classes out and the answer is inside the instrument's own scatter: the two jobs (14:21:36Z and 14:25:21Z, the same external `swe` sidecar reported as `stream=0` and `stream=1`) both resolved `reference: method=audio`, so the plugin forced `--vad webrtc` — which excludes **S43** (the VAD reading subtitles) by the plugin's own log line; no subtitle ruler was involved, so **S31**'s wrong-reference class does not apply; `--no-fix-framerate` is absent from argv, so the engine evaluated the PAL/NTSC ratios and ratio 1.0 won — a *whole-file* speedup is out, and no rescale was refused or written; and the `framerate:` line's absence is expected on the audio path (it is logged only when a subtitle reference was rescaled). The second job re-used the first job's `.npz`, so it is the same signal scored twice (identical `score=97950`), not a second measurement. What is left is the cue-level question the log cannot answer: the subtitle may be a *different cut* (piecewise structure is only investigated when `SplitPenalty > 0`, and the default is a single global offset) or the file being played may be the untouched original sidecar, which copy mode deliberately leaves in place. **Not determinable on this machine** — the media is not mounted here (only `/DATA/Media/Hermes-media` is) and the production server (172.17.0.4:8096, `FABJI-FLIX`) answers 401 without a key. The three measurements that settle it, and the exact commands, are in `knowledge/INVESTIGATION_false_trail_2026-09-18.md` §7: the cue-by-cue delta of `.swe.srt` against `.swe.SYNCED.srt` (constant 50 ms = the plugin applied what the engine said; a growing trend = not this cut), `ffprobe` for an embedded `swe` text track (a second, independent ruler), and an engine re-run with `--vad webrtc` against `--vad subs_then_webrtc`. Corpus context measured the same night: 1037 alignment lines in the day's log, 75,6 % of them exactly 0 ms, the modal non-zero answer ±40 ms (one 25 fps frame), +50 ms five times — and on a rig file whose answer is known, a 50 ms step is 0,7 % of the score curve inside a 120 ms flat top. The three settling commands are at `knowledge/INVESTIGATION_false_trail_2026-09-18.md:298`. |
| done | **P5-5** | high | **A job whose alignment failed still wrote a synced subtitle, because every ruler guard was keyed on a measurement a sparse subtitle cannot produce.** Found by the day's log read-out (N1), then reproduced in the harness. `AlignmentMetrics.MeasureSyncChange` is the only signal the 30 s subtitle-reference ceiling and the window-pin retry had, and it returns null when the subtitle has fewer than three cues or its cue count changes between the engine's input and output - so a forced/signs track, or the plugin's own sparse output, was guarded by nothing and the engine's raw answer went straight to the library. In the field that was 14 jobs on 2026-09-18 whose last alignment score is negative and whose written offsets sit at or near the +/-150 s window bound (job `3a8fbe3d...` Solsidan S09E01 `score=-41642 offset=-149,990 s against reference subtitle s:0` -> `bytes=98` over the library's own sidecar; `155dd60e...` `bytes=124`; `14c87ee5...` `bytes=39853`), with `change=unknown` on the rest. **Fix:** the engine's own reported offset - already parsed for the log line - is carried out of `RunEngineAttemptAsync` as an `EngineAttempt` record and is the fallback for both guards (`rulerDemandMs = measured?.ShiftMs ?? engineShiftMs`, `onCeilingShiftMs` likewise); the audio retry refreshes it so the guards never judge the answer that was just rejected, and that retry now parses its own score and offset (it previously inherited the failed attempt's). A measurable subtitle takes exactly the path it took before. **Reproduced and verified** by `tests/job_checks.cs` ("a sibling ruler pinned at the search window is discarded even when the subtitle is too sparse to measure"): a 2-cue sidecar against a sibling ruler reporting `offset seconds: -149.990`, `score: -41642.0`. Before: `status=Completed runs=1 outcome='only 2 cues in a 70-minute file ...' written=00:07:30,000` - the ruler's answer in the library, one engine run, no guard (suite 1045 PASS / 2 FAIL). After: `runs=2`, the second run against the audio (`/subsync/ref/` absent from its argv), `written=00:10:05,000` - the audio's answer (suite 1047 PASS / 0 FAIL). The source pin for the reference ceiling was re-pointed at the new expression rather than deleted. Shipped in 2.0.67 (beta) and checked as an installer sees it: the catalog reports 2.0.67.0, the 42 523 181-byte zip's MD5 is 4bb776542b600d2db29a052f39a958d1 and matches the catalog's checksum, the packaged meta.json and the DLL both carry 2.0.67.0, 174 bundled ffsubsync files, the refactor markers are 28/28 with both removals confirmed, and the assembly carries this release's own members and sentences (EngineAttempt as UTF-8 metadata, the fix's log sentence and the approved refusal sentence as UTF-16 literals, plus the two web markers ss-row.viewed and ss-run-counts) - python3 tests/backend/verify_release.py 2.0.67.0 reports OK. | `tests/job_checks.cs`; `tests/run_checks.py` (the re-pointed pin); log evidence in `knowledge/LOG_ANALYSIS_2026-09-18.md` sections 7.6/7.7 |
| open | **P5-6** | low | A re-synced `X.SYNCED.<lang>.srt` is written as `X.SYNCED.srt`: the marker strip removes the marker *and the fields after it*, so the plugin's own language-carrying output loses its language and lands as a second sidecar for that language — the opposite of S12's stated intent ("re-syncing `Film.S01E01.SYNCED.ukr.srt` updates that file"). Measured today twice: job `3a8fbe3d…` read `…/Solsidan.S09E01.SYNCED.nob.srt` and wrote `…/Solsidan.S09E01.SYNCED.srt` (`12:20:02.992Z` and `12:20:03.969Z`), and job `155dd60e…` the same for `S09E03`. | Those two job lines, verbatim, in the day's log; the naming that drops the language is `Jellyfin.Plugin.SubSync/Services/SyncedTargetNaming.cs:93`. |
| open | **P5-7** | medium | The rest of the day's log read-out, registered here as one pointer because each carries its own severity and verbatim evidence in the artifact: **N2** `change=unknown` on 16 written files (medium), **N3** one library folder unwritable so every job targeting it fails — 7 errors, `Black.Mirror.2011.S05.1080p.NF.WEB-DL.H.265…`, an environment fix (high), **N4** the queue lock held up to 30 483 ms (medium), **N5** the cue-index plan 60–72× off on the Solsidan shape, 20 WARN misses (medium), **N6** a fetch that did not cover its read, 20 times (low), **N7** `Thunder in My Heart - S01E03` unextractable by both routes, retried every run (medium), **N8** rescale ratios of 1,62× / 2,35× / 2,40× accepted on 5 jobs — not a framerate pair (medium), **N9** three page-created runs all labelled `Selection (285 files)` carrying 459/35/772 tasks (low), **N10** `batch history: restored 8 batch(es), 773 task(s)` byte-identical on all 8 startups of the day, including two after batches completed (medium), **N11** two startups with no teardown before them (low), **N12** a sidecar write raced a reader and lost (low), **N13** `/media\|/dev/sda2` measured at 3090,8 ms per read — storage, not code (medium), **N14** what the log cannot determine, named so it is not taken for clean. | `knowledge/LOG_ANALYSIS_2026-09-18.md:1038` ("New issues found"); the pointer's own severity is the middle of its range and each member's is named there, N3 (high) being the worst. |
| open | **P5-8** | medium | **A batch's tail runs one worker at a time when it is one volume, and that volume's read-latency class is decided once, under load, and never revisited.** The user's own observation ("at 90 % the speed slowed to 1 worker") from batch `8b8dd2d6274c4d3187c62b887b0170a9` (772 tasks): `running=8, limit=8` at 00:11-00:21Z, drifting 5 -> 4 -> 3 -> 2 from 00:22, and `queue: N queued, 1 running ... lane idle` from 00:40:19 to 00:57:25 - the last ~60 files ran one at a time with the configured limit at 8. Cause, from the log: every remaining job sits on `/media/synology|192.168.0.110:/volume1/JELLYFIN`, which the plugin holds at `1 concurrent media read(s) - this volume measured 169,8 ms per read, over 3 read(s), which is thrashing`. That figure was taken once (00:11:10, while 8 jobs were already reading) and is quoted identically in all 78 later `walk ceiling:` lines through 00:57 - nothing re-measures the read latency for the rest of the run, not even after 00:22 when that share was the only volume in use, while its *walks* on the same share were healthy (58,0 MB/s: `The Lighthouse (2019)`, 8562,3 MB in 154,7 s, 00:56:30). **Re-read on 2026-09-19, counting the tail's composition: half of this row's premise is refuted.** Cache-served jobs are not gated by this ceiling - `HasRoomOnItsVolume` returns true immediately for anything the scheduler does not classify as a media read (`SubSyncService.Scheduler.cs:384`; the classification itself is P5-10) - and the tail was not cheap work waiting behind a gate: 102 jobs started after 00:38Z, **62 of them `reference: method=subtitle` on the capped share**, in bursts a one-read ceiling cannot produce (36 starts in 60 s at 00:50Z, Solsidan S03E01-E10 at 1,1-1,4 s each; 15 in 60 s at 00:59Z). What the tail actually waited on was one *rescale* job at a time: each printed "the subtitle's span is 0,96x the reference's (a framerate pair) - rescaling" and then "the subtitle was stretched, so the stretch is tested against the film's own audio", i.e. a second full pass over the film's audio (45-95 s each: 00:41:01Z -> 00:42:34Z -> 00:43:53Z -> 00:44:49Z), classified heavy only because that file's speech cache was cold (P5-10), behind a ceiling no read ever corrected (P5-9). The row's second open question is closed rather than answered: **there was nothing to interleave** - the last ~60 files were all on `/media/synology` by composition ("90 queued across 67 media file(s)" down to "63 across 55"). Not fixed: read-out only. | the `walk ceiling:`/`queue:`/`dispatch:` lines of `/subsync-logs/subsync.log{,.1}` from 00:10Z to 00:57Z; the verdicts that decided the ceiling are `Jellyfin.Plugin.SubSync/Services/SubSyncService.Scheduler.cs:815` (thrashing) and `:829` (storage-bound) |
| done | **P5-9** | medium | **The ceiling's read-latency figure is never fed by a single real read: the profile is handed to a read policy only when its reader carries a path, and the extractor's reader never does.** `VolumeProfile.Observe(bytes, ms)` is meant to hear about every read a pass makes - `ReadPolicy.Observe` forwards each one (`Services/ReadPolicy.cs:466`), from the two timed read sites `Services/MkvSubtitleExtractor.cs:2998` (`RandomAccess.Read`) and `:3114` (`_stream.Read`) - and the profile keeps a 10-minute half-life weighted median over them. But the policy is only given a profile when the reader has a path (`Services/MkvSubtitleExtractor.cs:493`), and `BlobReader` is constructed without one at its only construction site (`Services/MkvSubtitleExtractor.cs:344`, `new BlobReader(stream)`, against `BlobReader(FileStream stream, string path = "")` at `:2831`), so `_volume` is null and every `Observe` is a silent no-op. Measured: every `walk ceiling:` line of 2026-09-18/19, on all four volumes, quotes exactly the one-shot probe's count (`this volume measured 13,8 ms per read, over 3 read(s)` at 03:29:38Z; `169,8 ms per read, over 3 read(s)` at 01:00:33Z), while the same share's own extraction pass measured `storage 1,15 ms per read and 2,7 MB/s (10 read(s) of this pass over 1 update(s))` at 00:42:34Z - two figures for one share, two orders of magnitude apart, and the ceiling keeps the stale one. **Fix (implemented and verified 2026-09-19, not shipped).** The path goes in with the stream - `reader = new BlobReader(stream, videoPath)` (`Services/MkvSubtitleExtractor.cs:344`) - so the policy the pass builds is handed its volume and the reads it was already making feed it. Reproduced before the fix at the call site the old check never reached (`tests/run_checks.py`: "volume profile 20 -> 20 sample(s) for 17/68/123/17 read call(s)", four FAILs with 1047 PASS), green after (20 -> 28/96/219/236 samples; 1051 PASS / 0 FAIL), and end to end on the rig (`tests/rig/run_scenario.py --scenario p59-read-feed`, S41's 231 ms tier): the ceiling line now reads `this volume measured 231,1 ms per read, over 16 read(s), which is thrashing`, where every line of the field run read `over 3 read(s)` or `nothing has measured this volume yet`. Not fixed by adding a probe and not adding one: the feed was there and dead. **Shipped in 2.0.68** (beta, `2c5f6f7`): the catalog serves 2.0.68.0, the zip is 42523821 bytes with MD5 `ba63abdc2034343eec57619a93393592` matching the manifest's checksum, and the packaged meta.json and DLL both carry 2.0.68.0 (`python3 tests/backend/verify_release.py 2.0.68.0`: OK). | `Services/MkvSubtitleExtractor.cs:344`, `:493`, `:2831`, `:2998`, `:3114`; `Services/ReadPolicy.cs:466`; `Services/SubSyncService.Scheduler.cs:849` (the ", over N read(s)" that quotes it); the `walk ceiling:` and `extract profile:` lines of `/subsync-logs/subsync.log{,.1}` 03:29:38Z-01:00:33Z |
| done | **P5-10** | medium | **`JobNeedsHeavyIo` asks whether a file's speech cache is cold, not whether the job will read the media, so a subtitle-ruled job is charged to its volume's ceiling for reads it never makes.** The scheduler passes `job => JobNeedsHeavyIo(job, headMode)` (`Services/SubSyncService.Scheduler.cs:1126`), which resolves to `!job.Stream.IsExternal -> true; else SyncJobMode.UsesSpeechCache(mode) && !SpeechIsCached(job)` (`Services/SubSyncService.Scheduler.cs:141`), and `HasRoomOnItsVolume` exempts anything not heavy (`Services/SubSyncService.Scheduler.cs:384`) - so that classification is the only thing between a job and the ceiling. In the tail of batch `8b8dd2d6274c4d3187c62b887b0170a9`, The Helicopter Heist S01E06's three queued jobs - each `extract: method=cache ... (no read: this file's subtitle is already out)`, each finishing in 1,3 s with "the subtitle's span is 0,95x the reference's, which is not a framerate pair - left for the alignment to report" - started 1,37 s apart (00:40:00.948Z, 00:40:02.316Z, 00:40:03.636Z) while the plan reported `starting 1, running 0, limit 8, queued 90` across 67 media file(s). They were shareable (`canShareMediaFile = ExtractionReady`) and read nothing, yet the wave took one per pass: the signature of a heavy candidate on a share held at `1 concurrent media read`. Same accounting family as **S42** (a walk charged to a job that never read the file), which folds in here. **Fix (implemented and verified 2026-09-19, not shipped).** The decision is now its own function, `SubSyncService.NeedsHeavyIo(readsMediaNow, isExternal, usesSpeechCache, speechCached, rulerIsTheAudio)` (`Services/SubSyncService.Scheduler.cs:152`), fed by `RulerWillBeTheAudio` (the pipeline's own picker over the same embedded-track list, memoised per file) and by a flag the job sets the moment a pass actually reads the media (`Services/SubSyncService.JobPipeline.cs:1272` for an engine run against the audio, `:2583` for the rescale check) - which is what answers the caveat: the plan still predicts, and the job corrects it mid-run. **S42 folded in:** the walk a run attributes is charged to the read (`Services/SubSyncService.JobPipeline.cs:1341`, `if (readTheMedia)`), so an audio-referenced run whose speech came from the cache stops reporting a walk of the whole file. Verified by six pure-function checks in `tests/run_checks.py` and four source pins that fail against the previous revision of both files (evaluated: all four FAIL on `HEAD`'s text, all four PASS on the fix); suite 1061 PASS / 0 FAIL. **Measured end to end** (`tests/rig/run_scenario.py --scenario p510-sibling-wave`: one file with three external sidecars on the shimmed volume, held at one read by S41's 231 ms tier, `ultimate` mode, queued in one batch). Before: the pair ran one after the other - the second sidecar's engine started at 10:11:41.759Z, after the first had completed at 10:11:41.277Z, and the refusal that held it is named in the log (`walk ceiling: holding /opt/data|/dev/nvme0n1p2 at 1 concurrent media read(s) ... which is thrashing`). After: both start within 7 ms (10:14:41.316Z / 10:14:41.323Z) and finish together (10:14:42.100Z), with **no ceiling line at all** - nothing was refused because nothing needed the audio. The scenario opens its window after the job that measures the volume, so that job cannot be mistaken for the pair under test. **Shipped in 2.0.68** (beta, `2c5f6f7`): the catalog serves 2.0.68.0, the zip is 42523821 bytes with MD5 `ba63abdc2034343eec57619a93393592` matching the manifest's checksum, and the packaged meta.json and DLL both carry 2.0.68.0 (`python3 tests/backend/verify_release.py 2.0.68.0`: OK). | the `dispatch:` / `framerate:` / `extract:` lines of `/subsync-logs/subsync.log.1` 00:40:00Z-00:40:04Z; `Services/SubSyncService.Scheduler.cs:141`, `:384`, `:1126` |
| done | **P5-11** | low | **The marker on the row a plain click views was grey-black, where the user wants the colour picks use.** Report, verbatim: "the single episode marked look is black which is wrong, selected files should always have the same colour blue we alwasys use". The `.ss-row.viewed` rule added for P5-1 was a neutral white tint - a `rgba(255,255,255,.055)` background, a `rgba(255,255,255,.3)` bar and no name colour of its own - which on the dark theme renders as grey-black: a dead-looking row rather than a marked one. It now names the theme's primary colour exactly as `.ss-row.selected` does. Measured in a real browser with real input (`tests/gui/p5-viewed.js`: a left click on one row, a right click on another): before, the clicked row computed `bg=rgba(255, 255, 255, 0.055)`, `bar=rgba(255, 255, 255, 0.3) 3px`, `name=rgb(221, 221, 221)` while the picked row computed `bg=color(srgb 0 0.643137 0.862745 / 0.14)`, `bar=rgb(0, 164, 220) 3px`, `name=rgb(0, 164, 220)`; after, the clicked row computes the picked row's values exactly - one blue, no grey, and the class is still `viewed` (so picking logic is untouched). **Shipped in 2.0.68** (beta, `2c5f6f7`): the catalog serves 2.0.68.0, the zip is 42523821 bytes with MD5 `ba63abdc2034343eec57619a93393592` matching the manifest's checksum, and the packaged meta.json and DLL both carry 2.0.68.0 (`python3 tests/backend/verify_release.py 2.0.68.0`: OK). | `tests/gui/p5-before-look.json` and `tests/gui/p5-after-look.json`; shots `tests/gui/shots/p5-{before,after}-look-{1-clicked,2-picked}.png`; `Jellyfin.Plugin.SubSync/Web/subsyncMain.html:67`; the pin "the viewed row wears the theme primary, not a neutral tint" in `tests/run_checks.py` |

**What the day was clean about, in one line each** (the numbers are in the artifact): 1000 engine runs, all
`exit=0`; 0 cancellations and 0 leaked processes across 6 teardowns; 791 of 976 completed jobs wrote nothing because
the subtitle was already in sync; the extraction cache answered 745 of 976 jobs with no read at all; every batch that
finished is fully accounted for, with no job id queued twice and no engine run twice for one job.
