...
| **B10** | low | static | `FinaliseStats(stats)` is called before `stats.ReadCalls` and `stats.TotalMs` are assigned (`MkvSubtitleExtractor` 345-347 and 359-361), so the summary it builds cannot carry them. |
...
| open | **B2** | medium | Shared extraction pass returns success while omitting a requested track. **Reproduced 2026-09-13 on the D17 mixed-index shape** (`tests/backend/p4_probe.py b2-mixed-shared /tmp/d17fx/d17-mixed.mkv 0 1 2`): track 5, whose index is half-located, is **absent from `results` while `TryExtractMany` returns `true` and `reason` is empty**; the same tracks extracted alone give 803 cues for track 5 (8 and 726 for tracks 4 and 6, which the shared pass matches exactly). It is not E2's root cause any more - the shared pass reads what its index locates and deliberately leaves a track it cannot serve to the caller (`Extract`'s own comment says so) - but the contract is silent about it: the only signal is the `tracks=N/M` field in the log line. The caller does recover: `SubSyncService` extracts a track alone when its subtitle is absent from the cache (the path at `SubSyncService.cs` **shared-pass cache miss**), so no subtitle is lost today, which is why this is medium and not high. Fix would be the contract saying what it could not serve (a missed-tracks field or a `reason` naming them) rather than a behaviour change in the pass. | `tests/backend/p4_probe.py`, mode `b2-mixed-shared`; before/after counts in the Phase 4 report. |
