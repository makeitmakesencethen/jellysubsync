...
| done (added) | C2 (this session) | low | piecewise (split-penalty) alignment with the plugin's own guard: fixture `step-out.srt` (20 s step at 17-min midpoint) reads as two flat segments; the linear ratio is refused by `IsRescaleAcceptable` (ratio > 1,02); `PiecewiseHolds` holds and `far-out.srt` is refused. 6 new checks (`tests/run_checks.py`) + 5 tested via `engine_sampling_bench.py`. Keeps the engine flag (`PiecewiseArgs(config)`). Verdict: in, already enabled, verified. |
...
| open (held) | R1 / per-volume cap | medium | per-volume walk cap is the design this session's measurements contradict the repo's deliberate invariant against (`tests/run_checks.py` asserts `no per-volume budget`); the measurement (S28 / S28-server-levels) shows the NAS collapses at limit 8 (65,6 files/h vs 122-135 at 1-4, per-file 19x worse) and the queueing regime applies. A tiered cap (threshold on `MsPerCall()`) answers both SSD and NAS and keeps the invariant for fast/unknown volumes, but it reverses 3 checks + 1 reflection check; needs an explicit user decision before it ships. Held. |
