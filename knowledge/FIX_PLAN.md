...
| open | **B31** | medium | An unguarded scheduler loop: one exception inside `PumpAsync` ends it silently, and the lane's unexpected failures go to Jellyfin's log, so a stalled batch has no line in the plugin log - indistinguishable from a dead plugin. See the Matrix B row for the observed 2.0.28 shape and what would prove the trigger |  |
...
| partial | **B31** | medium | **code verified, trigger unproven** | The scheduler's loop is unguarded and a batch that stalls says nothing in the plugin log. `PumpAsync` (`SubSyncService` 2398-2595) has **no try/catch around its loop**: one unexpected exception ends the scheduler, and `WakePump()` (2370-2383) only restarts it when something else wakes it, so if nothing can finish - jobs waiting on extraction while the lane is gone or blocked - the batch freezes with no line in the plugin log at all. The lane has the mirror-image gap: its unexpected-exception handler (1746) writes to Jellyfin's server log via `_logger.LogWarning`, not to `PluginLog`, so a lane failure is invisible in the file this project reads runs out of. **Observed shape (2.0.28 batch `ebe7d657220d4bcf9df5925b475d19da`)**: 52 jobs queued, 38 dispatch lines ending `dispatch: starting 1, running 7, limit 8, queued 1` at 15:09:50Z, then 23 completed / 0 failed / 0 cancelled and **29 jobs with no terminal line**, the last at 15:11:29Z - and the file was being appended normally right up to that second (a job completion is the last line), so this is not a log-write failure: `PluginLog.Append` re-opens the path per write, has no disable-on-error flag and never throws, the volume has 79 GB free and no rotation happened (1.29 MB against a 4 MB limit). **What would prove it**: Jellyfin's own log around 15:11:29Z - the plugin's `Extraction lane failed for <file>` warning, or an unobserved-task trace - and the batch's state on the plugin page (`/SubSync/Active`), which distinguishes a stalled scheduler from a dead one. **Fix (not written yet)**: guard the pump loop with a catch-all that logs `PluginLog.Error` and continues, mirror the lane's exception warning into the plugin log, and emit a heartbeat line every few minutes while jobs are outstanding so a stall reads as a stall. | Log `/subsync-logs/subsync.log` lines 4236-4688 (2.0.28 slice), 0 ERROR lines, no `startup:` line after 15:07:56Z; code at `SubSyncService` 2398-2595, 2370-2383, 1746. |
