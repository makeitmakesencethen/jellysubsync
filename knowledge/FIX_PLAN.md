...
| open | **S28** | high | the audio reference handed to the engine is a **symlink to the whole media file**, so every audio-ruler job makes ffsubsync demux and decode the entire film instead of a compact audio file - a full read of a 5-15 GB file, off the share, while the extraction lane and seven sibling workers read the same volume. Same trap `AGENTS.md` already names for subtitle references ("Never let ffsubsync pull a subtitle stream out of the video... measured on an 8.2 GB episode: 12,5 s and 8218 MB read"); for audio it is still in place. Not inherent to the method: 232 audio-ruler engine runs have a **median 35,3 s**, episodes (~1 GB) take 14-35 s while films (5-15 GB) take 57-111 min - a 50-100x anomaly against a ~3x length ratio - and the parent process sat at 0,2-2,9 % CPU, i.e. waiting | `SpeechCache.CreateReferenceLink`: `File.CreateSymbolicLink(linkPath, videoPath)` - the reference *is* the media file (the symlink exists only so the `.npz` lands where `Harvest` can take it). Field evidence: `00:34:24.977 [c4d7d3ae] ffsubsync start: ... reference=a:0 args=.../speech-cache/<key>.mkv -i <sidecar> -o <tmp> --max-offset-seconds 150 --max-subtitle-seconds 10 --vad subs_then_webrtc --ffmpeg-path /usr/lib/jellyfin-ffmpeg/ffmpeg --reference-stream a:0 --serialize-speech`, exit 91,3 min later (Midsommar); Hereditary 111+ min; Oppenheimer 65,5; The Lighthouse 57,2. Per-read latency on the same share ranged 2,14 ms to 1613 ms within minutes. **Fix direction (not scoped)**: extract one compact audio-only reference per file into the local cache (`ffmpeg -map 0:a:0 -vn -ac 1 -ar 16000 -c:a flac`, ~150-300 MB for a film) and hand the engine that instead of a symlink to the media, keeping the `.npz` harvest and the once-per-file caching. A/B (in progress on this machine with `tests/backend/slowread.so`): (A) the plugin's invocation with the symlink reference, (B) the same with the local audio-only file |
