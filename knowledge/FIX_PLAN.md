...
| open | **E3** | low | `MkvExtractionStats.ClustersVisited` counts a walked cluster twice - once for the cluster header/timecode read the cue point makes and once for the walk itself - so the counter disagrees with the reference extraction on any partly located index: 15 visits for 10 cue points on `MKV_FIX_MIXED` (5 located + 5 walked), and 1205 for 803 cue points on the D17 mixed-index fixture where the NEbml reference reports 803. Nothing depends on the number (the per-cue read assertion uses `ReadCalls`), but the log line is read by the user and by these checks, so it should say one visit per cluster. | Rig stats on `MKV_FIX_MIXED` and the D17 mixed-index fixture, after `9bbec18`. |
