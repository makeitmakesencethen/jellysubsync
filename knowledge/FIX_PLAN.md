...
| state | id | sev | what | evidence |
|---|---|---|---|---|
| open | **S45** | medium | after a wrong-cut ruler is discarded, the audio retry can write the *discarded ruler's* answer and report it as the audio's | **Found while writing the characterization tests for `RunSyncJob` (2026-09-16), reproduced deterministically, not fixed in that pass (tests only).** When the reference ceiling discards a subtitle ruler, the audio re-alignment is given `tempOutput` - the same path the reference run just wrote to - and the gate afterwards is `if (audioExit == 0 && File.Exists(tempOutput))`. An audio run that exits 0 without writing (ffsubsync suppresses its write when the shift is under its threshold, which is the case exactly when the subtitle already matches the film) therefore finds the *discarded ruler's* output still in place, takes the success branch, measures it, writes it and says "aligned against the audio". Measured in the harness on a 40-cue fixture: the reference run wrote a `+45 s` answer, the audio run wrote nothing, and the sidecar written to the library carried the reference's cue (`00:10:45,000 --> 00:10:47,000`) under the outcome `the file's own subtitle track is not the same cut, so this was aligned against the audio · +45000 ms offset`. With the stale file removed the same case refuses with the sentence the refusal is supposed to produce, so the difference is the leftover file and not the audio's answer. Same family as **S22** (a sentence that does not describe what happened), with a written sidecar attached. Repair (later pass): delete or give the audio retry its own output path, so `File.Exists(tempOutput)` can only be true of a file that retry wrote. | `tests/job_checks.cs` case `p8-stale-output`, asserted by the check `P8: with the reference run's output left in place, that stale file is accepted as the audio's answer` (`tests/run_checks.py`), which fails when the audio-retry gate is broken (`P8-stale` in `tests/backend/mutation_job_checks.py`); the companion case `p8-refusal` shows the refusal that happens without the stale file. |
