...
| open | **F3** | high | Item-scoped endpoints have no per-item access check (IDOR, read and write) | **Real.** The controller's only gate is the class-level `[Authorize]` (`Api/SubSyncController.cs:16`); there is no reference to `User`, `HasPermission`, `GetItemById`, `Claims` or `IUserManager` anywhere in it (grep: no hits). `:51` hands `GetSubtitles(Guid itemId)` straight to `_syncService.ListSubtitles(itemId)` and `:74` hands `SyncSubtitle` straight to `_syncService.StartSync(request.ItemId, request.SubtitleIndex, request.Mode)` - so any authenticated account can read any item's subtitle list and, more seriously, queue a sync against any item id it names, and sync jobs write subtitle files. Discovery is the other half: F4 shows every job on the server, with item ids, to the same account. |
