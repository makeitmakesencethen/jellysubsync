...
- **Multi-subtitle modes** — when one run covers several subtitles: **Normal** (one at a time, audio analysed every run), **Parallel** (several media files at once, worker count configurable), **Fast** (the audio is analysed once per media file and that speech signal is reused for its other subtitles), or **Ultimate** (parallel + reuse — the fastest way through a large selection). All four produce identical alignments; parallel waves never put two workers on the same file.
