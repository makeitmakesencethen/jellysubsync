...
- **Multi-subtitle modes** — when one run covers several subtitles: **Normal** (one at a time, audio analysed every run), **Parallel** (several at once, worker count configurable), or **Fast** (the audio is analysed once per media file and that speech signal is reused for its other subtitles — identical results, ~2-3x less work per extra subtitle). Fast helps when several subtitles share one file: a movie with many tracks, or re-syncing the same file later.
