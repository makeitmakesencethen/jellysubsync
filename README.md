...
- **Fast embedded extraction** — for Matroska files the plugin reads an embedded text subtitle through the container's own structures (SeekHead → Cues → only the clusters that hold the track, reading block headers and skipping every payload) instead of demuxing the container. Measured on a 63 GB Blu-ray-shaped remux with the cue index at the end of the file: **28 ms and 0.2 MB read versus 612 ms and 996 MB** for the earlier index reader, and versus a full 63 GB demux for ffmpeg. If the index says nothing about the track, the plugin scans cluster metadata only (payloads skipped) rather than handing the file to ffmpeg — same file, 1.8 MB read. Image codecs and laced blocks still fall back to ffmpeg, and the whole path can be switched off in the config.
...
- **Waves spread over your disks** — when a batch spans several volumes, each wave picks one file per volume before doubling up, because workers reading the same device finish together rather than apart. This is a preference, not a limit: a library on a single disk still runs at the full worker count. Several subtitles of the *same* file never start before that file's audio analysis is cached, so no two workers race to build it.
- **Automatic sync strategy** — the mode is chosen from the work, so there is nothing to configure: one subtitle on one file stays sequential; several subtitles of one file analyse the audio once and then run **in parallel** against that cached signal; several files run in parallel with each file's analysis reused. Manual overrides exist in Settings for troubleshooting. Every strategy produces identical alignments.
