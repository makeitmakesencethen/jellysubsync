...
- **Storage-aware parallel runs** — parallel work is scheduled per volume: at most one task reads heavily from a given disk at a time, so four workers on one NAS no longer crawl together. Tasks whose audio analysis is already cached read nothing and run fully in parallel.
